/* Write a C program to implement doubly linked list using DMA. OR */
/* Write a C program that performs different operations on the doubly linked list using DMA. */
...
{
	int data;
	struct doubly_linklist *prev, *next;
};

struct doubly_linklist *start = NULL, *node = NULL;
...
void append()	//create or create_list
{
	...
	if(start == NULL)
	{	
		start = (struct doubly_linklist *) malloc(sizeof(struct doubly_linklist));
		start->prev = NULL;
		start->data = num;
		start->next = NULL;
	}
	else
	{
		...
		node->next = (struct doubly_linklist *) malloc(sizeof(struct doubly_linklist));
		node->next->prev = node;
		node->next->data = num;	
		node->next->next = NULL;
	}
